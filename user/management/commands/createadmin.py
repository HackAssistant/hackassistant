...
from django.contrib.auth.management.commands.createsuperuser import Command as SuperUserCommand, PASSWORD_FIELD, \
    NotRunningInTTYException, exceptions, get_default_username, CommandError, capfirst, getpass, validate_password, \
    os, sys
...
class Command(SuperUserCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--domain",
            default=getattr(settings, 'HOST', 'localhost') or 'localhost',
            help='Specifies the domain of the app to use. Default is "localhost".',
        )
        return super().add_arguments(parser)

    def hash_password(self, domain, password):
        hasher = hashlib.sha512()
        hasher.update((domain + ':' + password).encode('utf-8'))
            ...
            if options["interactive"]:
                ...
                while PASSWORD_FIELD in user_data and user_data[PASSWORD_FIELD] is None:
                    ...
                    try:
                        ...
                        if response.lower() != "y":
                            continue
                    user_data[PASSWORD_FIELD] = self.hash_password(domain, password)
            else:
